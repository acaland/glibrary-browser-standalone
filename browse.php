...
isc.DataSource.create({   
    dataFormat:"json",
    ID:"columVisibleDS",    
    dataURL:urlPrefix + "glibrary_conexion_update.php?task=RECORDFIELD",    
    fields:[
    	{name:"name"}
    ]
});
...
isc.DataSource.create({   
    dataFormat:"json",
    ID:"columHideDS",    
    dataURL:urlPrefix + "glibrary_conexion_update.php?task=HIDDENFIELD",    
    fields:[
    	{name:"name"}
    ]
});
isc.RestDataSource.create({
    ...
    fetchDataURL:urlPrefix + "glibrary_conexion_update.php?task=LISTRECORDS"
   //v updateDataURL:"glibrary_conexion_update.php?task=UPDATE"
...
isc.RestDataSource.create({
    ...
    ID:"viewDS",    
    fetchDataURL:urlPrefix + "glibrary_conexion_update.php?task=LISTRECORDS",
    updateDataURL:urlPrefix + "glibrary_conexion_update.php?task=UPDATE"
...
isc.DataSource.create({   
    dataFormat:"json",
    ID:"surlDS",    
    dataURL:urlPrefix + "glibrary_conexion_update.php?task=LISTSURL",
...
isc.DataSource.create({   
    dataFormat:"json",
    ID:"relationsDS",
    dataURL:urlPrefix + "glibrary_conexion_update.php?task=LISTRELATION",
...
isc.DataSource.create({   
    ...
    ID:"repositoryDS",   
    fields:[    
    	{type: "text", title:"Repository Path", name:"repository"},
    	{name:"thumb",title:"Thumbnail",type:"link"},
    	{name:"rep_name",title:"Repository Name",type:"text"}
     ],
...
isc.DataSource.create({   
    ...
    fields:[    
     ...
     dataURL:urlPrefix + "glibrary_conexion_update.php?task=GETFILTER"
...
isc.DataSource.create({   
    ...
    fields:[    
     ...
     dataURL:urlPrefix + "glibrary_conexion_update.php?task=GETFILTERATTRI"
...
isc.IButton.create({
    ...
    autoDraw:false,
    click:"document.location.href='upload.php'"
...
isc.IButton.create({
    ...
    height: 40,
     icon: Page.getSkinDir()+"/images/actions/save.png",
    autoDraw:false,
    click:"document.location.href='logout.php'"
...
isc.DataSource.create({    
   dataFormat:"json",
    ID:"userDS",    
    dataURL:urlPrefix + "glibrary_conexion_update.php?task=GETUSER",    
    fields:[
    	{name:"name"}
    ]
});
...
isc.DataSource.create({   
    ...
    dataURL: urlPrefix + "glibrary_conexion_update.php?task=GETTREETYPES&rep="+repositoryField
...
isc.DataSource.create({   
    ...
    dataURL: urlPrefix + "glibrary_conexion_update.php?task=GETTREECOLLECT&rep="+repositoryField
...
isc.TreeGrid.create({
		   ...
		   showDropIcons:false,		 
		   selectionType:"single",
		   //folderIcon: Page.getSkinDir()+"/images/actions/remove.png",
		   fields: [
			{name: "name"}
		   ],		     
...
isc.HTMLPane.create({
	ID:"pageHeader",
    width:"100%", height: "115", showEdges:false,
...
isc.HLayout.create({
    ...
    members:[
		...
		members:[
		 ...
		 height:175,
			...
			sections:[
			 ...
			 {ID:"firstRight",title:"Filter attributes", autoShow:true,items:[
			     isc.Canvas.create({
				ID:"findPane1",
				height:"30",
				autoDraw:false,
			  ...
			  {ID:"secondRight",title:"Values", autoShow:true, items:[
				isc.Canvas.create({
				  ID:"findPane2",
				  height:"100%",
				  autoDraw:false,
		...
		showResizeBar:false,		
		sections:[
			{title:"List",items:[amgaGrid],canCollapse: true, headerControls : [filterButton1,filterButton2,displayButton,userForm]}
			]
...
isc.TabSet.create({
	...
	members: [	
	  isc.SectionStack.create({
	    	...
	    	height:"100%",	   
            sections:[
               {ID: "imageSection",left:100,title:"Thumbnail", autoShow:true, items:[isc.Img.create({ID:"imgTabs",autoDraw: false, imageType: "center", imageHeight: 300, height: 300, overflow: "auto", layoutAlign: "center"})]},
               {title:"Details", autoShow:true, items:[entryViewer]}
	       ]
        })]
	})},
      {title:"Edit generic attributes", pane:editGeneric, ID:"editTab1", width:70},
	...
	function (response, data, request) {
	   if(data){
		...
		if((data[0].Thumb!=null)&&(data[0].Thumb!="")){
			...
			if (data[0].ThumbURL != "")
				thumbSrc = data[0].ThumbURL;
			else
				thumbSrc="http://glibrary.ct.infn.it/glibrary_new/glibrary_conexion_update.php?task=GETTHUMB&rep="+repositoryField+"&idImg="+data[0].Thumb;
			//alert($data[0].Thumb);
			//modalWindow.setTitle(typeof data[0].Thumb);
			//isc.say(thumbSrc);
			// imgTabs.setSrc(data[0].Thumb);
			imgTabs.setSrc(thumbSrc);
...
function onChangeRepertory(){
	if ((repositoryField=="deroberto2")||(repositoryField=="deroberto")) 
		pageHeader.setContentsURL("header_dr.php");
	else 
		pageHeader.setContentsURL("header.php");
	treeTypes.fetchData({rep: repositoryField});	
	treeCollections.fetchData({rep: repositoryField});		
	filterHide(true);
	amgaGrid.setData([]);
...
function filterShow(){
   ...
   if((treeTypes.getSelection()!="")||(treeCollections.getSelection()!="")){
	if(LayOutVisible){
		VLayout1.setShowResizeBar(true);
		switch(numFilterShow){
			case 0:
...
function saveColumns(){
	...
	RPCManager.send("null","CallbackSaveVisual(data)",{actionURL: urlPrefix + "glibrary_conexion_update.php?task=SAVEVISUAL",httpMethod:"GET",showPrompt:true,useSimpleHttp:true,params:{node: nodeField,rep: repositoryField,names:nameStr,widths:widthStr,collID:idCollect}});
